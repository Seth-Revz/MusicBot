    ...
    async def cmd_save(self, player):
        ...
        if player.current_entry and not isinstance(player.current_entry, StreamPlaylistEntry):
            ...
            if url not in self.autoplaylist:
                ...
                return Response(self.str.get('cmd-save-success', 'Added <{0}> to the autoplaylist.').format(url), delete_after=30)
            else:
    ...
    async def cmd_unsave(self, player, url=None):
        ...
        if url or (player.current_entry and not isinstance(player.current_entry, StreamPlaylistEntry)):
            ...
            if url in self.autoplaylist:
                ...
                return Response(self.str.get('cmd-unsave-success', 'Removed <{0}> from the autoplaylist.').format(url), delete_after=20)
            else:
    ...
    async def _cmd_play(self, message, player, channel, author, permissions, leftover_args, song_url, head=False):
        ...
        async with self.aiolocks[_func_() + ':' + author.id]:
            ...
            if position == 1 and player.is_stopped:
                ...
                if head:
                    reply_text %= (btext, 1, ftimedelta(time_until))
                else:
